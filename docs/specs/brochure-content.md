# Bandurria — Contenido extraído del Brochure 2026

> Fuente: `docs/Brochure 2026.pdf`
> Usado como insumo para construir el sitio web.

---

## Identidad / Hero

- **Nombre:** BANDURRIA
- **Claim principal:** "EXPERTOS EN COMUNICACIÓN VISUAL"
- **Tagline:** "SOMOS BANDURRIA Y NOS GUSTA ENCONTRAR SOLUCIONES SIMPLES A DESAFÍOS COMPLEJOS"
- **Propuesta diferencial:** 100% IN HOUSE DE TU COMUNICACIÓN VISUAL

---

## Servicios

### 1. Impresión en Gran Formato — Cartelería y Gran Formato

Equipos de impresión de hasta **320 cm de ancho**. Tinta UV de alta resistencia apta para interior y exterior.

**Productos:**

- Banners con portabanner y tipo Roll Up
- Back de prensa

**Sustratos disponibles:**

- Lonas vinílicas (front, backlight, blackout, mesh)
- Vinilos autoadhesivos (base blanca, base gris, cristal, microperforado)
- Papeles de distintos gramajes
- Lienzo canvas
- Cuerina
- Imán
- Stickers

---

### 2. Impresión sobre Materiales Rígidos / POP

Impresión a través de **cama plana** sobre múltiples sustratos.

**Sustratos rígidos:**

- PVC
- PAI
- Foamboard
- Plástico corrugado
- PET cristal
- Cartulina
- Cartón

**Servicios de corte:**

- Corte estándar
- Medio corte
- Corte a 45°
- 100% automatizado

**Tintas:** CMYK y Blanco

**Producción especializada:**

- Fabricación de exhibidores en plástico corrugado
- Impresión sobre materiales especiales provistos por el cliente: chapa galvanizada, acero, plásticos, vidrio, acrílico, madera, goma eva, alfombra, corcho

**Productos para punto de venta (PDV):**

- Colgantes
- Stoppers
- Salientes
- Cenefas
- Headers
- Cubrepallet
- Movies

---

### 3. Impresiones Offset y Digital

**Productos:**

- Flyers
- Folletos
- Posters
- Afiches para vía pública
- Dípticos / Trípticos / Cuadripticos
- Tarjetas
- Carpetas de presentación
- Encuadernaciones
- Catálogos
- Revistas
- Sobres

---

## Tecnología

Equipamiento propio (100% in-house):

| Equipo                   | Detalle                                                      |
| ------------------------ | ------------------------------------------------------------ |
| Plotters de gran formato | Entre 160 y 320 cm de ancho                                  |
| Tintas                   | Ecosolventes y UV, aptas interior/exterior, alta durabilidad |
| Cama plana               | Alta producción                                              |
| Equipo de corte          | Suizo Zund de última generación                              |
| Soldadoras               | Para lonas de gran tamaño                                    |
| Ojalilladoras            | Neumáticas                                                   |
| Laminadora               | Frío / Calor                                                 |

---

## Contacto

| Canal               | Dato                            |
| ------------------- | ------------------------------- |
| Teléfono / WhatsApp | 11 2408 0830                    |
| Email               | produccion@bandurriadeco.com.ar |
| Ubicación           | Chacarita, Capital Federal      |

---

## Sugerencias para la página web

### Secciones recomendadas

1. **Hero** — Nombre + claim + tagline + CTA WhatsApp
2. **Nosotros** — Propuesta de valor "soluciones simples a desafíos complejos" + "100% in house"
3. **Servicios** — 3 cards principales: Gran Formato · Materiales Rígidos/POP · Offset y Digital
4. **Detalle de servicios** — Expandido por servicio con sustratos/productos como lista visual
5. **Tecnología** — Grid de equipos con íconos o tabla
6. **Clientes** — Logos de empresas que confían (actualmente vacío en brochure, añadir cuando disponibles)
7. **Contacto** — Teléfono, email, ubicación + botón WhatsApp

### Palabras clave para SEO

- Impresión gran formato Buenos Aires
- Cartelería Chacarita CABA
- Impresión POP punto de venta
- Bandurria gráfica
- Comunicación visual CABA
