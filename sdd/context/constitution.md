...
- **Rol arquitectónico:** Sitio web institucional de la empresa (ver `sdd/global.json` → `project`). Es una SPA estática (sin backend propio) que comunica la propuesta de valor de la empresa — servicios, tecnología, clientes y contacto — y hospeda el brochure comercial 2026. Su audiencia son clientes actuales y potenciales que llegan por buscadores, redes o el equipo comercial.
...
- Presentar la identidad, los servicios y las capacidades técnicas de la empresa en una SPA pública.
- Publicar y mantener el brochure comercial (contenido 2026) como página navegable dentro del sitio.
- Mantener performance, accesibilidad y SEO de un sitio público estático (build de Vite desplegable como estático).
- Ofrecer canales de contacto directo (formulario/enlaces, WhatsApp) hacia los sistemas externos de la empresa.

**No hacemos:**

- No hay backend, base de datos ni autenticación propios: todo el contenido es estático o versionado en el repo.
- No se persisten datos de usuarios ni se procesan pagos dentro de este proyecto.
- No es un CMS: el contenido se edita por código/deploy, no por panel de administración.
- No aloja lógica de negocio de otros productos de la empresa.

### 1.3 Principios de Diseño

1. **Contenido como dato:** los textos y listados (servicios, stats, clientes) viven en módulos de datos tipados bajo `src/data/`, no incrustados a mano en el JSX de cada sección.
2. **Secciones componibles:** cada bloque de página es un componente autónomo en `src/components/sections/`; las páginas solo componen secciones y no contienen lógica de presentación propia.
3. **Estático primero:** ninguna feature puede requerir un servidor propio; si necesita datos externos, se consume un servicio de terceros documentado en la spec.
4. **La animación no bloquea el contenido:** el sitio debe ser legible y navegable sin depender de Framer Motion ni de JS de scroll (respetar `prefers-reduced-motion`).
5. **El build es la barrera mínima:** `pnpm lint` + `pnpm build` (que incluye `tsc -b`) tienen que estar en verde; sin `any` en código de producto. Nota: `strict` todavía **no** está activo en `tsconfig.app.json` — ver deuda en la constitución del subproyecto.
...
El sistema es un **repositorio standalone** (sin Nx): una única app React cuyo código vive en la raíz del repo (`src/`, `public/`, `index.html`). El identificador `apps/{project}` es solo la convención con la que los registros SDD refieren a ese subproyecto lógico.
...
| ----------- | ----- | ------------------------------------------------ | ----------------------------------------- |
| apps/{project}  | react | Sitio institucional + brochure 2026 (código en la raíz del repo) | `sdd/context/apps/{project}/constitution.md` |
...
- Repo **standalone** (sin Nx) con **pnpm** como package manager y **Vite 8** como build tool.
